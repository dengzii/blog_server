...
func TestGetCallback(t *testing.T) {
	...
	DB.Callback().Create().Register("gorm:test_callback", func(scope *Scope) { scope.Set("gorm:test_callback_value", 1) })
	...
	DB.Callback().Create().Replace("gorm:test_callback", func(scope *Scope) { scope.Set("gorm:test_callback_value", 2) })
	callback = DB.Callback().Create().Get("gorm:test_callback")
	if callback == nil {
		t.Errorf("`gorm:test_callback` should be non-nil")
	}
	...
	DB.Callback().Create().Register("gorm:test_callback", func(scope *Scope) { scope.Set("gorm:test_callback_value", 3) })
	callback = DB.Callback().Create().Get("gorm:test_callback")
	if callback == nil {
		t.Errorf("`gorm:test_callback` should be non-nil")
	}
