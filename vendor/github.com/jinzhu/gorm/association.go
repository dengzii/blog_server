	...
	if relationship.Kind == "belongs_to" {
		...
		if relationship.Kind == "many_to_many" {
			...
			if sourcePrimaryKeys := scope.getColumnAsArray(sourceForeignFieldNames, scope.Value); len(sourcePrimaryKeys) > 0 {
				...
				association.setErr(Delete(relationship.JoinTableHandler, newDB))
			}
...
func (association *Association) Count() int {
	...
	case "many_to_many":
		query = JoinWith(relationship.JoinTableHandler, query, scope.Value)
...
func (association *Association) saveAssociations(values ...interface{}) *Association {
	...
	saveAssociation := func(reflectValue reflect.Value) {
		...
		if relationship.Kind == "many_to_many" {
			association.setErr(Add(relationship.JoinTableHandler, scope.NewDB(), scope.Value, reflectValue.Interface()))
		} else {
