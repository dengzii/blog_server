...
func TestUpdate(t *testing.T) {
	...
	DB.Model(&product4).Update("price", Expr("price + ? - ?", 100, 50))
...
func TestUpdates(t *testing.T) {
	...
	updatedAt4 := product4.UpdatedAt
	DB.Model(&product4).Updates(map[string]interface{}{"price": Expr("price + ?", 100)})
	var product5 Product
	DB.First(&product5, product4.Id)
...
func TestUpdateColumn(t *testing.T) {
	...
	DB.Model(&product4).UpdateColumn("price", Expr("price + 100 - 50"))
	var product5 Product
	DB.First(&product5, product4.Id)
	if product5.Price != product4.Price+100-50 {
