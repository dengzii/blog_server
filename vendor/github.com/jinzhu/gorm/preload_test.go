...
func TestNestedPreload1(t *testing.T) {
	...
	if err := DB.Preload("Level2").Preload("Level2.Level1").Find(&got, "name = ?", "not_found").Error; err != ErrRecordNotFound {
		t.Error(err)
	}
}
...
func TestNestedManyToManyPreload(t *testing.T) {
	...
	if err := DB.Preload("Level2s.Level1s").Find(&got, "value = ?", "not_found").Error; err != ErrRecordNotFound {
		t.Error(err)
	}
}
...
func TestNestedManyToManyPreload2(t *testing.T) {
	...
	if err := DB.Preload("Level2.Level1s").Find(&got, "value = ?", "not_found").Error; err != ErrRecordNotFound {
		t.Error(err)
	}
}
...
func TestNestedManyToManyPreload3(t *testing.T) {
	...
	var gots []*Level3
	if err := DB.Preload("Level2.Level1s", func(db *DB) *DB {
...
func TestPreloadManyToManyCallbacks(t *testing.T) {
	...
	DB.Callback().Query().After("gorm:query").Register("TestPreloadManyToManyCallbacks", func(scope *Scope) {
		called = called + 1
	})
