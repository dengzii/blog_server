...
func (scope *Scope) Quote(str string) string {
	if strings.Contains(str, ".") {
		...
		for _, str := range strings.Split(str, ".") {
			newStrs = append(newStrs, Quote(str))
		}
...
func (scope *Scope) AddToVars(value interface{}) string {
	...
	return BindVar(len(scope.SQLVars))
}
...
func (scope *Scope) Exec() *Scope {
	...
	if !scope.HasError() {
		if result, err := Exec(scope.SQL, scope.SQLVars...); scope.Err(err) == nil {
...
func (scope *Scope) Begin() *Scope {
	if db, ok := scope.SQLDB().(sqlDb); ok {
		if tx, err := Begin(); scope.Err(err) == nil {
...
func (scope *Scope) limitAndOffsetSQL() string {
	return LimitAndOffsetSQL(scope.Search.limit, scope.Search.offset)
}
		...
		if fromField != nil {
			if relationship := fromField.Relationship; relationship != nil {
				if relationship.Kind == "many_to_many" {
					...
					scope.Err(JoinWith(joinTableHandler, tx, scope.Value).Find(value).Error)
...
func (scope *Scope) createJoinTable(field *StructField) {
	if relationship := field.Relationship; relationship != nil && relationship.JoinTableHandler != nil {
		joinTableHandler := relationship.JoinTableHandler
		joinTable := Table(scope.db)
		if !HasTable(joinTable) {
			...
			for idx, fieldName := range relationship.ForeignFieldNames {
				if field, ok := scope.FieldByName(fieldName); ok {
					...
					sqlTypes = append(sqlTypes, scope.Quote(relationship.ForeignDBNames[idx])+" "+DataTypeOf(foreignKeyStruct))
			...
			for idx, fieldName := range relationship.AssociationForeignFieldNames {
				if field, ok := toScope.FieldByName(fieldName); ok {
					...
					foreignKeyStruct.TagSettingsSet("IS_JOINTABLE_FOREIGNKEY", "true")
					foreignKeyStruct.TagSettingsDelete("AUTO_INCREMENT")
					sqlTypes = append(sqlTypes, scope.Quote(relationship.AssociationForeignDBNames[idx])+" "+DataTypeOf(foreignKeyStruct))
...
func (scope *Scope) modifyColumn(column string, typ string) {
	scope.db.AddError(ModifyColumn(scope.QuotedTableName(), scope.Quote(column), typ))
}
...
func (scope *Scope) addIndex(unique bool, indexName string, column ...string) {
	if HasIndex(scope.TableName(), indexName) {
		return
	}
...
func (scope *Scope) addForeignKey(field string, dest string, onDelete string, onUpdate string) {
	...
	keyName := BuildKeyName(scope.TableName(), field, dest, "foreign")

	if HasForeignKey(scope.TableName(), keyName) {
		return
	}
...
func (scope *Scope) removeIndex(indexName string) {
	RemoveIndex(scope.TableName(), indexName)
}
...
func (scope *Scope) autoMigrate() *Scope {
	...
	if !HasTable(tableName) {
		...
		for _, field := range scope.GetModelStruct().StructFields {
			if !HasColumn(tableName, field.DBName) {
				if field.IsNormal {
					sqlTag := DataTypeOf(field)
	...
	for _, field := range scope.GetStructFields() {
		if name, ok := field.TagSettingsGet("INDEX"); ok {
			...
			for _, name := range names {
				if name == "INDEX" || name == "" {
					name = BuildKeyName("idx", scope.TableName(), field.DBName)
				}
				name, column := NormalizeIndexAndColumn(name, field.DBName)
		...
		if name, ok := field.TagSettingsGet("UNIQUE_INDEX"); ok {
			...
			for _, name := range names {
				if name == "UNIQUE_INDEX" || name == "" {
					name = BuildKeyName("uix", scope.TableName(), field.DBName)
				}
				name, column := NormalizeIndexAndColumn(name, field.DBName)
